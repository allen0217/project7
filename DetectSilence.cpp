...
bool detectSilence(Mat frame, Point location, Mat ROI, CascadeClassifier cascade)
{
	...
	cascade.detectMultiScale(ROI, mouths, 1.1, 3, 0, Size(35, 35));
...
int detect(Mat frame,
	...
	*/
	...
	for (int i = 0; i < nfaces; i++) {
		...
		int x1 = face.x;
		//1.5--8 error  2--8 error  1.65--8 error
		//int y1 = face.y + face.height/2;
		int y1 = face.y + face.height / 1.65;
		float ax = 0.15*face.width;
		float ay = 0.1*face.height / 2;
		//Rect lower_face =  Rect(x1, y1, face.width, face.height/2);
		Rect lower_face = Rect(x1 + ax, y1 + ay, face.width - ax, (face.height / 2) - ay);
